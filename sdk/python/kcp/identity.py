...
def _load_wordlist(lang: str = "en") -> list[str]:
    ...
    raise ImportError(
        "KCP identity requires 'mnemonic' package for secure key recovery. Install with: pip install mnemonic"
    )
...
def generate_mnemonic(strength: IdentityStrength = IdentityStrength.STANDARD, language: str = "en") -> str:
    """
    ...
    try:
        ...
        raise ImportError("KCP identity requires 'mnemonic' package. Install with: pip install mnemonic") from exc
...
def create_identity(
    strength: IdentityStrength = IdentityStrength.STANDARD, language: str = "en", passphrase: str = ""
) -> KCPIdentity:
    """
...
def recover_identity(mnemonic: str, passphrase: str = "", language: str = "en") -> KCPIdentity:
    ...
    if not validate_mnemonic(mnemonic, language):
        raise ValueError(
            "Frase de recuperação inválida. Verifique se digitou todas as palavras corretamente e na ordem certa."
        )
...
def save_identity(identity: KCPIdentity, keys_dir: Path, save_mnemonic: bool = False) -> None:
    """
...
def export_identity(keys_dir: Path, output_path: Path, password: str | None = None) -> None:
    """
...
def import_identity(input_path: Path, keys_dir: Path, password: str | None = None) -> dict:
    """
    ...
    if content[:7] == b"KCPKEY1":
        ...
        try:
            ...
            raise ValueError("Senha incorreta ou arquivo corrompido.") from exc
