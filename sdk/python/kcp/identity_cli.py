...
def wizard_create_identity(lang: str = "en") -> None:
    ...
    4. Saving keys
    """
    from .identity import IdentityStrength, create_identity, save_identity
...
""".format(
    ...
    identity = create_identity(strength=strength, language=lang, passphrase=passphrase)
    ...
    if not confirm("Você anotou as palavras?" if lang == "pt" else "Have you written down the words?"):
        print_warning(
            "Por favor, anote as palavras antes de continuar."
            if lang == "pt"
            else "Please write down the words before continuing."
        )
    ...
    if word3 != words[2] or word7 != words[6] or word11 != words[10]:
        print_error(
            "Palavras incorretas. Verifique sua anotação." if lang == "pt" else "Incorrect words. Check your notes."
        )
        if not confirm("Continuar mesmo assim?" if lang == "pt" else "Continue anyway?"):
            return
    else:
...
def import_backup(input_path: str | None = None, lang: str = "en") -> None:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="kcp identity", description="Gerenciar identidade KCP" if lang == "pt" else "Manage KCP identity"
    )

    subparsers = parser.add_subparsers(dest="command", help="Comando")

    # create
    subparsers.add_parser("create", help="Criar nova identidade" if lang == "pt" else "Create new identity")

    # recover
    subparsers.add_parser(
        "recover", help="Recuperar identidade da frase" if lang == "pt" else "Recover identity from phrase"
    )

    # show
    subparsers.add_parser("show", help="Mostrar identidade atual" if lang == "pt" else "Show current identity")

    # export
    export_parser = subparsers.add_parser("export", help="Exportar backup" if lang == "pt" else "Export backup")
    export_parser.add_argument("-o", "--output", help="Arquivo de saída")

    # import
    import_parser = subparsers.add_parser("import", help="Importar backup" if lang == "pt" else "Import backup")
    import_parser.add_argument("-f", "--file", help="Arquivo de backup")

    # Language flag
    parser.add_argument("--lang", "-l", choices=["en", "pt"], default=lang, help="Idioma / Language")
